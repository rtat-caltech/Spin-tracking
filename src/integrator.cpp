...
int integrateMagnusCFET(double t0, double tf, double3& y, const double3& p_old,
	...
	while (t < tf) {
		...
		B4 = findCrossTerm(t+GL5::X4*dt, OPT.B0, OPT.E, OPT.gamma, t0, tf, p_old, p_new, v_old, v_new);
		B5 = findCrossTerm(t+GL5::X5*dt, OPT.B0, OPT.E, OPT.gamma, t0, tf, p_old, p_new, v_old, v_new);
		...
		k9 = (CFET8::G35 * B1 + CFET8::G34 * B2 + CFET8::G33 * B3 + CFET8::G32 * B4 + CFET8::G31 * B5) * dt;
		k8 = (CFET8::G45 * B1 + CFET8::G44 * B2 + CFET8::G43 * B3 + CFET8::G42 * B4 + CFET8::G41 * B5) * dt;
		k7 = (CFET8::G55 * B1 + CFET8::G54 * B2 + CFET8::G53 * B3 + CFET8::G52 * B4 + CFET8::G51 * B5) * dt;
		k6 = (CFET8::G61 * B1 + CFET8::G62 * B2 + CFET8::G63 * B3 + CFET8::G64 * B4 + CFET8::G65 * B5) * dt;
		k5 = (CFET8::G51 * B1 + CFET8::G52 * B2 + CFET8::G53 * B3 + CFET8::G54 * B4 + CFET8::G55 * B5) * dt;
		k4 = (CFET8::G41 * B1 + CFET8::G42 * B2 + CFET8::G43 * B3 + CFET8::G44 * B4 + CFET8::G45 * B5) * dt;
		k3 = (CFET8::G31 * B1 + CFET8::G32 * B2 + CFET8::G33 * B3 + CFET8::G34 * B4 + CFET8::G35 * B5) * dt;
		k2 = (CFET8::G21 * B1 + CFET8::G22 * B2 + CFET8::G23 * B3 + CFET8::G24 * B4 + CFET8::G25 * B5) * dt;
		k1 = (CFET8::G11 * B1 + CFET8::G12 * B2 + CFET8::G13 * B3 + CFET8::G14 * B4 + CFET8::G15 * B5) * dt;
		...
		k5 = (CFET6::H15 * B1 + CFET6::H14 * B2 + CFET6::H13 * B3 + CFET6::H12 * B4 + CFET6::H11 * B5) * dt;
		k4 = (CFET6::H25 * B1 + CFET6::H24 * B2 + CFET6::H23 * B3 + CFET6::H22 * B4 + CFET6::H21 * B5) * dt;
		k3 = (CFET6::H31 * B1 + CFET6::H32 * B2 + CFET6::H33 * B3 + CFET6::H34 * B4 + CFET6::H35 * B5) * dt;
		k2 = (CFET6::H21 * B1 + CFET6::H22 * B2 + CFET6::H23 * B3 + CFET6::H24 * B4 + CFET6::H25 * B5) * dt;
		k1 = (CFET6::H11 * B1 + CFET6::H12 * B2 + CFET6::H13 * B3 + CFET6::H14 * B4 + CFET6::H15 * B5) * dt;
